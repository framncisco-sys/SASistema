...
function App() {
  ...
  // --- FUNCIONES DE CARGA ---
  const cargarClientes = () => { fetch('http://127.0.0.1:8000/api/clientes/').then(r=>r.json()).then(setListaClientes).catch(console.error); };
  useEffect(() => { cargarClientes(); }, []);
  useEffect(() => { if (clienteActivo && periodo) fetch(`http://127.0.0.1:8000/api/finanzas/resumen/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setDatosFiscales); }, [clienteActivo, periodo, vistaActual]);
  useEffect(() => { if (vistaActual === "libroCompras" && clienteActivo) fetch(`http://127.0.0.1:8000/api/compras/listar/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setListaCompras); }, [vistaActual, clienteActivo, periodo]); 
  useEffect(() => { if ((vistaActual === "libroVentasCCF" || vistaActual === "libroVentasCF") && clienteActivo) { const tipo = vistaActual === "libroVentasCCF" ? "CCF" : "CF"; fetch(`http://127.0.0.1:8000/api/ventas/listar/?nrc=${clienteActivo}&periodo=${periodo}&tipo=${tipo}`).then(r=>r.json()).then(setListaVentas); } }, [vistaActual, clienteActivo, periodo]);
  ...
  const guardarNuevoCliente = () => {
      ...
      fetch('http://127.0.0.1:8000/api/clientes/crear/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre: nuevoNombre, nrc: nuevoNrc, nit: nuevoNit, es_importador: nuevoEsImportador }) })
  ...
  const guardarCompraEnBackend = (irALibro) => {
    ...
    let url = 'http://127.0.0.1:8000/api/compras/crear/'; let metodo = 'POST';
    if (idEdicion) { url = `http://127.0.0.1:8000/api/compras/actualizar/${idEdicion}/`; metodo = 'PUT'; }
  ...
  const guardarRetencionEnBackend = () => {
      ...
      fetch('http://127.0.0.1:8000/api/retenciones/crear/', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(paqueteRet) }).then(async res => { if (res.ok) { alert("✅ Retención Agregada"); resetFormularios(); setVistaActual("dashboard"); } else alert("❌ Error al guardar retención."); });
  };
  ...
  const borrarSeleccionados = (tipo) => { if (seleccionados.length === 0) return; if (window.confirm(`⚠️ ¿Eliminar ${seleccionados.length} items?`)) { const promesas = seleccionados.map(id => fetch(`http://127.0.0.1:8000/api/${tipo}/borrar/${id}/`, { method: 'DELETE' })); Promise.all(promesas).then(() => { alert("🗑️ Eliminados."); setSeleccionados([]); if (tipo === 'compras') fetch(`http://127.0.0.1:8000/api/compras/listar/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setListaCompras); else { const tipoV = vistaActual === "libroVentasCCF" ? "CCF" : "CF"; fetch(`http://127.0.0.1:8000/api/ventas/listar/?nrc=${clienteActivo}&periodo=${periodo}&tipo=${tipoV}`).then(r=>r.json()).then(setListaVentas); } fetch(`http://127.0.0.1:8000/api/finanzas/resumen/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setDatosFiscales); }); } };
  const borrarVenta = (id) => { if (window.confirm("⚠️ ¿Eliminar esta venta?")) { fetch(`http://127.0.0.1:8000/api/ventas/borrar/${id}/`, { method: 'DELETE' }).then(res => { if (res.ok) { fetch(`http://127.0.0.1:8000/api/ventas/listar/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setListaVentas); fetch(`http://127.0.0.1:8000/api/finanzas/resumen/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setDatosFiscales); } }); } };
  const borrarCompra = (id) => { if (window.confirm("⚠️ ¿Eliminar?")) { fetch(`http://127.0.0.1:8000/api/compras/borrar/${id}/`, { method: 'DELETE' }).then(res => { if (res.ok) { fetch(`http://127.0.0.1:8000/api/compras/listar/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setListaCompras); fetch(`http://127.0.0.1:8000/api/finanzas/resumen/?nrc=${clienteActivo}&periodo=${periodo}`).then(r=>r.json()).then(setDatosFiscales); } }); } };
          ...
          <>
                ...
                <div style={{ background: 'white', padding: '20px', borderRadius: '10px', borderTop: '5px solid #27ae60', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
                    ...
                    <div style={{ background: '#fcfcfc', padding: '15px', borderRadius: '10px', marginTop:'20px', border:'1px solid #eee' }}>
                        ...
                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px'}}>
                             <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/csv-compras/?nrc=${clienteActivo}&periodo=${periodo}`} style={{padding:'8px', background:'#eee', border:'none', cursor:'pointer'}}>Compras</button>
                             <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/csv-ventas-ccf/?nrc=${clienteActivo}&periodo=${periodo}`} style={{padding:'8px', background:'#eee', border:'none', cursor:'pointer'}}>Ventas CCF</button>
                             <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/csv-ventas-cf/?nrc=${clienteActivo}&periodo=${periodo}`} style={{padding:'8px', background:'#eee', border:'none', cursor:'pointer'}}>Ventas CF</button>
                             <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/csv-163/?nrc=${clienteActivo}&periodo=${periodo}`} style={{padding:'8px', background:'#eee', border:'none', cursor:'pointer'}}>Percepciones (163)</button>
                             <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/csv-162/?nrc=${clienteActivo}&periodo=${periodo}`} style={{padding:'8px', background:'#eee', border:'none', cursor:'pointer'}}>Retención 1% (162)</button>
                             <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/csv-161/?nrc=${clienteActivo}&periodo=${periodo}`} style={{padding:'8px', background:'#eee', border:'none', cursor:'pointer'}}>Retención TC (161)</button>
                        </div>
                    </div>
                </div>
              </div>
            ...
            {vistaActual === "libroCompras" && (
                <div style={{ background: 'white', padding: '20px', borderRadius: '10px' }}>
                    ...
                    <div style={{display:'flex', gap:'10px', marginBottom:'20px'}}>
                         <button onClick={() => { window.location.href = `http://127.0.0.1:8000/api/reportes/csv-compras/?nrc=${clienteActivo}&periodo=${periodo}`; }} style={{ padding: '10px', background: '#7f8c8d', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>📄 Descargar CSV Compras</button>
                         <button onClick={() => { window.location.href = `http://127.0.0.1:8000/api/reportes/pdf-compras/?nrc=${clienteActivo}&periodo=${periodo}`; }} style={{ padding: '10px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>🖨️ Imprimir PDF</button>
                    </div>
            ...
            {(vistaActual === "libroVentasCCF" || vistaActual === "libroVentasCF") && (
                <div style={{ background: 'white', padding: '20px', borderRadius: '10px' }}>
                    ...
                    <div style={{display:'flex', gap:'10px', marginBottom:'20px'}}>
                         <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/csv-ventas-${vistaActual === "libroVentasCCF" ? 'ccf' : 'cf'}/?nrc=${clienteActivo}&periodo=${periodo}`} style={{ padding: '10px', background: '#9b59b6', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>📄 Descargar CSV MH</button>
                         {vistaActual === "libroVentasCCF" && <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/pdf-ventas-ccf/?nrc=${clienteActivo}&periodo=${periodo}`} style={{ padding: '10px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>🖨️ PDF CCF</button>}
                         {vistaActual === "libroVentasCF" && <button onClick={() => window.location.href = `http://127.0.0.1:8000/api/reportes/pdf-ventas-cf/?nrc=${clienteActivo}&periodo=${periodo}`} style={{ padding: '10px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>🖨️ PDF CF</button>}
                    </div>
                    <table style={{width:'100%', borderCollapse:'collapse'}}>
                     ...
                     <div style={{border: '3px dashed #3498db', padding: '50px', textAlign:'center', borderRadius: '15px', backgroundColor: '#f0f8ff', cursor: 'pointer', position: 'relative'}}>
                        ...
                        <input type="file" multiple accept=".json"
                            onChange={(e) => {
                                ...
                                fetch('http://127.0.0.1:8000/api/sistema/procesar-lote/', { method: 'POST', body: formData }).then(r => r.json()).then(data => setCargaAnalisis(data)).catch(err => alert("Error analizando"));
                                e.target.value = null; 
                            }}
